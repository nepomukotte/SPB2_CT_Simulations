...
int iNSummedPixel = 8; //number of pixel that are connected to one MUSIC

int iTelID = 0;


ofstream *fConfigFile = new ofstream("SPB2HorizSplit.cfg");     //! ascii file


void WriteCameraGeometryHorizSplit()
{
...
TGeoTranslation *tr = new TGeoTranslation(xPixelCoord,yPixelCoord, 0.);
   ...
   for(int x = 0; x<iModulesInRow[0];x++)
     {
...
///   gGeoManager->SetTopVisible();
   ...
   for(UInt_t m=0; m<vYCoordModule.size();m++)
   {
	 ...
	 {
		 *fConfigFile <<"* SIPMPARS "<<iTelID<<"  "<<uPixID<<" 14400 0.01"<<endl;
		uPixID++;
	 }
   ...
   *fConfigFile<<endl<<endl;
   ...
   for(UInt_t m=0; m<vYCoordModule.size();m++)
   {
     for(UInt_t p=0; p<vYCoordPixelInModule.size();p++)
	 {
		 ...
		 *fConfigFile <<"* PMPIX "<<iTelID<<" 0 1  "<<uPixID<<" "<<setw(10)<< setprecision(5)<<xc<<"   "<<yc<<" "<<fPixelPitch/sqrt(2.0)*10<<" 0.0"<<endl;
		vXCoordPixel.push_back(xc);
		vYCoordPixel.push_back(yc);

		//The sum groups
		int sumgroupid = uPixID/iNSummedPixel;
                 //vSumGroupIDinModule[p]+vYCoordPixelInModule.size()/iNSummedPixel*m;
   ...
   *fConfigFile<<endl<<endl<<endl;
   ...
   for(UInt_t g=0;g<vXCoordGroup.size();g++)
   {
       ...
       for(UInt_t ng=0;ng<vXCoordGroup.size();ng++)
	   {
		   ...
		   if(fabs(vYCoordGroup[ng]-yg)<=(3*fPixelPitch*10) && (fabs(vXCoordGroup[ng]-xg)<1)   && ng!=g )
